...
public class DoctorDao {
	...
	public DoctorBean doctorLogin(String email, String password) {
		...
		Query qe = se.createQuery("from DoctorBean d where d.email=:x and d.password=:y").setParameter("x", email)
	...
	public boolean saveDoctor(DoctorBean dregister) {
		Session se = sessionFactory.openSession();
	...
	public DoctorBean updateDoctor(DoctorBean register) {
		Session se=sessionFactory.openSession();
		Transaction tx=se.beginTransaction();
		DoctorBean pb=(DoctorBean)se.merge(register);
		tx.commit();
		se.close();
		return pb;
	}

	public List<AppointmentBean> appointments(String specialization) {
		Session se=sessionFactory.openSession();
		Query qe= se.createQuery("from AppointmentBean a where a.specialist = :x").setParameter("x", specialization);
		List li =qe.list();
		return li;
	}
